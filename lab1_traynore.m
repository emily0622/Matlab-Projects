% try open('x = [.5, 1.0, 2.3, 4.0, 4.333, 5.];     %These are arbitrary values
%          ?
...
N=101;
d = (7-(-3))/(N-1); %find delta
x = zeros(N,1); %preallocate the array
y = zeros(N,1); %preallocate the array

for n=1:N
    x(n) = d*n -3; %find x value
    y(n) = abs(x(n))*x(n); %plug x into y function
end
...
z = sin(2 * pi * x);    %NOTE:  Multiplcation by the scalar 2*pi, does not 
                        ...
                        %       because the result is the same whether
                        %       using matrix multiplication or regular mult

plot(x,y,x,z)           %Verify that these are the same results (they are)
xlabel('x');
ylabel('f(x)');
...
clear y;
tic; %using a for loop
y=zeros(N,1);
for e=1:N
    y(e) = atan(e*d -2)
end
toc; %2.313051 seconds
y(1)

clear y;
tic; %using vectorization
y= atan(x)
toc; %0.01150 seconds

%vectorization is much faster on a large scale
%note: 5000001 broke my Matlab and so I had to scale down the number
...
plot(x,y,x,z)           %Verify that these are the same results ok
xlabel('x');
ylabel('f(x)');
...
t = linspace(1,5,100);
%general solu: y=(t+c)/e^t^2
c1=-exp(1)-1; %these are the constant for the three different init values
c2=-1;
c3=exp(1)-1;
y1 = (t+c1)./exp(t.^2); %plug constants into general solution
y2 = (t+c2)./exp(t.^2);
y3 = (t+c3)./exp(t.^2);
plot(t,y1,t,y2,t,y3); %and plot yay
xlabel('x')
ylabel('y')
title('ODE with 3 init conditions')
...
plot(x, sin(x),'--', x, cos(x),'-.', x, exp(-x.^2),':',x,0.1.*x,'o',x,0.2.*x,'-.',x,0.3.*x,'x');   
legend('sin', 'cos', 'exp(-x^2)','0.1x','0.2x','0.3x');
...
fprintf('Here are some values of the function: f(x) = (x^3 - abs(x)) * ln(x^2+1)');
fprintf(' f(%g) = %g\n', 0, f(0));
fprintf(' f(%g) = %g\n', 4, f(4));
fprintf(' f(%g) = %g\n', -5, f(-5));
...
fprintf('Exercise 7');
fprintf(' f( %g, %g) = %g', 0,0,f(0,0));
fprintf(' f( %g, %g) = %g', (pi/2),1,f(pi/2,1));
fprintf(' f( %g, %g) = %g', -1,-1,f(-1,-1));
...
% 100 points equally spaced from -2 to 2, inclusive
...
%general solution y = exp(-sin(x) + x^3 + c)
%implicit equation for y(0) = 1, exp(x^3 -sin(x))
f = @(x) exp(x^3 -sin(x)); 
...
%When y=0 the function is constant:y=0
%when the initial condition is y>0 the solution forms an open up parabola
%when the initial condition is y<0 the solution looks like an open up
%parabola but at y=0 meets a horizontal asymptote so the sides curve along
%the asymptote
%as x approaches infinity, the positive y values continue to have direction
%fields leading the solution away from y=0, the negative values continue to
%have direction fields that converge to y=0, and if y=0 x=0
%the solution is symmetric about the y-axis because y is squared so the
%left and right sides have the equivalent y values
...
%the constant are when y_0=-1 or 1 as x approaches infinity
%if-2<= y_0<=-1 it approaches -1
%if -1<y_0<=2 it appraoches 1
%if y>2 or y<-2, the values are unpredicatable and cross eachother with the
