...
%solution:
%y(t) = ((sin(t)^2)/(2cos(t)) + - (1/2cos(t))

% Construct the exact solution
tt = linspace(0,3.1415,50);
...
% Construct the exact solution using the vectorized x values ->soln.x
yy = ((sin(soln.x).^2)./(2.*cos(soln.x))) - (1./(2.*cos(soln.x)));
...
%the max error is 3.36e-05
%I think this is the max error cause the gradient is the largest in
%solution
%since the numerical approach compares the y_j to y_(j+h) the largest
%difference should be during the largest change in gradient
%% Exercise 4
%
...
%warning: failure at t=5.0660e-01. Unable to meet integration tolerance
%without reducing the step size below the smallest value allowed (1.77e-15)
%at time t.
%This means that at t=5.066e-01 and t=5.066e-01 +1.77e-15 the difference in solutions is still larger than the
%tolerance, and the tolerance cannot be reduced
...
% We set up our initial approximation phi_0 = 5:
...
for i = 1:N
    func=subs(f,y,phi);    % prepare function to integrate: y -> previous phi %(i)
    func=subs(func,t,s);      % variable of integration is s, so we need to change
                              % t -> s
...
picard=ezplot(phi,[0,1.5]); %phi(N+1)
set(picard,'Color','green');         % set the color of the graph to green
